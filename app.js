...
function checkBtnHandler(e) {
  ...
  if (bdayStr !== "") {
    ...
    if (isPalindrome) {
      outputBox.innerText = `Someone just got lucky! Your birthday is a palindrome!🥳🥳`;
    } else {
      const [counter, nextDate] = getNextPalindromeDate(date);
      outputBox.innerHTML = `The next palindrome date is ${nextDate.day}-${nextDate.month}-${nextDate.year}, date missed you by ${counter} days! Guess whose lose it is?👼👼 `;
    }
  }
}
